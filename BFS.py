...
def solvePuzzle_depth(srcLayout, destLayout):
    ...
    while len(stack_layouts) > 0:
        ...
        lst_shifts = g_shifts[ind_slide]#当前可进行交换的位置集合
    ...
    while g_layouts[curLayout] != -1:#存入路径
        curLayout = g_layouts[curLayout]
        lst_steps.append(curLayout)
    lst_steps.reverse()
    return 0, lst_steps
...
if __name__ == "__main__":
    ...
    if retCode != 0:
        print("Target layout is not reachable.")
    else:
